...
for n=1:DipoleNum
    ...
    if strcmpi(Type_d(n),'Px')==1
        ...
        Ex(n,:)=KE*(-((y-dy).^2+(z-dz).^2)./r1./r1.*g1r1+g2r1 + ((y-dy).^2+(z+dz).^2)./r2./r2.*g1r2-g2r2)*Px(n);
        Ey(n,:)=KE*((x-dx).*(y-dy)./r1./r1.*g1r1 - (x-dx).*(y-dy)./r2./r2.*g1r2)*Px(n);
        Ez(n,:)=KE*((z-dz).*(x-dx)./r1./r1.*g1r1 - (z+dz).*(x-dx)./r2./r2.*g1r2)*Px(n);
        Hx(n,:)=zeros(N, 1)*Px(n);
        Hy(n,:)=KH*(-(z-dz)./r1.*g3r1 + (z+dz)./r2.*g3r2)*Px(n);
        Hz(n,:)=KH*((y-dy)./r1.*g3r1 - (y-dy)./r2.*g3r2)*Px(n);
